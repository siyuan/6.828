...
boot_alloc(uint32_t n)
{
	...
	if ((uint32_t)(nextfree + ROUNDUP(n, PGSIZE)) > (uint32_t)KADDR(npages * PGSIZE-1))
...
page_alloc(int alloc_flags)
{
	...
	if (page_free_list) {
		...
		if (alloc_flags & ALLOC_ZERO)
			memset(KADDR(((uint32_t)result-(uint32_t)pages)/sizeof(struct Page)*PGSIZE), 0, PGSIZE);
	}
	else
		result = NULL;
	return result;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (pgdir[PDX(va)])
		...
		struct Page *ptepg;
		ptepg = page_alloc(ALLOC_ZERO);
		if (ptepg == NULL)
			return NULL;
		else {
			ptepg->pp_ref++;
			pgdir[PDX(va)] = page2pa(ptepg) | PTE_U | PTE_P;
			return (pte_t *)page2kva(ptepg)+PTX(va);
		}
	}
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	int i;
	pte_t *ptetmp;
	for (i=0; i<=(ROUNDDOWN(pa+size, PGSIZE) - ROUNDDOWN(pa, PGSIZE)); i+=PGSIZE) {
		ptetmp = pgdir_walk(pgdir, (uintptr_t *)(ROUNDDOWN(va, PGSIZE) + i), 1);
		*ptetmp = (pa + i) | perm | PTE_P;
	}
}
...
page_insert(pde_t *pgdir, struct Page *pp, void *va, int perm)
{
	// Fill this function in
	pte_t *ptetmp;
	ptetmp = pgdir_walk(pgdir, va, 1);
	if (ptetmp == NULL)
		return -E_NO_MEM;
	pp->pp_ref++;
	if(*ptetmp)
		page_remove(pgdir, va);
	*ptetmp = page2pa(pp) | perm | PTE_P;
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	// Fill this function in
	pte_t *ptetmp;
	ptetmp = pgdir_walk(pgdir, va, 0);
	if (ptetmp == NULL)
		return NULL;
	if (pte_store)
		*pte_store = ptetmp;
	return pa2page(PTE_ADDR(*ptetmp));
}
...
page_remove(pde_t *pgdir, void *va)
{
	...
	struct Page *pgtmp;
	pte_t *ptetmp;
	pgtmp = page_lookup(pgdir, va, &ptetmp);
	if (pgtmp) {
		page_decref(pgtmp);
		*ptetmp = 0;
		tlb_invalidate(pgdir, va);
